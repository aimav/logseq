...
return{startState:function(){return{indentStack:null,indentation:0,mode:!1,sExprComment:!1,sExprQuote:!1}},token:function(stream,state){null==state.indentStack&&stream.sol()&&(state.indentation=stream.indentation());if(stream.eatSpace())return null;var returnType=null;switch(state.mode){case "string":processEscapedSequence(stream,{token:'"',state});returnType="string";break;case "symbol":processEscapedSequence(stream,{token:"|",state});returnType="symbol";break;case "comment":var next;for(returnType=
!1;null!=(next=stream.next());){if("#"==next&&returnType){state.mode=!1;break}returnType="|"==next}returnType="comment";break;case "s-expr-comment":if(state.mode=!1,"("==stream.peek()||"["==stream.peek())state.sExprComment=0;else{stream.eatWhile(/[^\s\(\)\[\]]/);returnType="comment";break}default:if(next=stream.next(),'"'==next)returnType=state.mode="string";else if("'"==next)"("==stream.peek()||"["==stream.peek()?"number"!=typeof state.sExprQuote&&(state.sExprQuote=0):stream.eatWhile(/[\w_\-!$%&*+\.\/:<=>?@\^~]/),
returnType="atom";else if("|"==next)returnType=state.mode="symbol";else if("#"==next)if(stream.eat("|"))returnType=state.mode="comment";else if(stream.eat(/[tf]/i))returnType="atom";else if(stream.eat(";"))state.mode="s-expr-comment",returnType="comment";else{next=null;var hasExactness=!1,hasRadix=!0;stream.eat(/[ei]/i)?hasExactness=!0:stream.backUp(1);stream.match(/^#b/i)?next=isBinaryNumber:stream.match(/^#o/i)?next=isOctalNumber:stream.match(/^#x/i)?next=isHexNumber:stream.match(/^#d/i)?next=isDecimalNumber:
stream.match(/^[-+0-9.]/,!1)?(hasRadix=!1,next=isDecimalNumber):hasExactness||stream.eat("#");null!=next&&(hasRadix&&!hasExactness&&stream.match(/^#[ei]/i),next(stream)&&(returnType="number"))}else if(/^[-+0-9.]/.test(next)&&isDecimalNumber(stream,!0))returnType="number";else if(";"==next)stream.skipToEnd(),returnType="comment";else if("("==next||"["==next){returnType="";for(hasExactness=stream.column();null!=(hasRadix=stream.eat(/[^\s\(\[;\)\]]/));)returnType+=hasRadix;0<returnType.length&&indentKeys.propertyIsEnumerable(returnType)?
...
typeof state.sExprComment?"comment":"number"==typeof state.sExprQuote?"atom":returnType},indent:function(state){return null==state.indentStack?state.indentation:state.indentStack.indent},fold:"brace-paren",closeBrackets:{pairs:'()[]{}""'},lineComment:";;"}});CodeMirror.defineMIME("text/x-scheme","scheme")})}
