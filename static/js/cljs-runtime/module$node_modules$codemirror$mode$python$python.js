...
commonBuiltins;void 0!=parserConf.extra_keywords&&(i=i.concat(parserConf.extra_keywords));void 0!=parserConf.extra_builtins&&(myBuiltins=myBuiltins.concat(parserConf.extra_builtins));var py3=!(parserConf.version&&3>Number(parserConf.version));if(py3){var identifiers=parserConf.identifiers||/^[_A-Za-z\u00A1-\uFFFF][_A-Za-z0-9\u00A1-\uFFFF]*/;i=i.concat("nonlocal None aiter anext async await breakpoint match case".split(" "));myBuiltins=myBuiltins.concat(["ascii","bytes","exec","print"]);var stringPrefixes=
RegExp("^(([rbuf]|(br)|(rb)|(fr)|(rf))?('{3}|\"{3}|['\"]))","i")}else identifiers=parserConf.identifiers||/^[_A-Za-z][_A-Za-z0-9]*/,i=i.concat(["exec","print"]),myBuiltins=myBuiltins.concat("apply basestring buffer cmp coerce execfile file intern long raw_input reduce reload unichr unicode xrange None".split(" ")),stringPrefixes=RegExp("^(([rubf]|(ur)|(br))?('{3}|\"{3}|['\"]))","i");var keywords=wordRegexp(i),builtins=wordRegexp(myBuiltins);return{startState:function(basecolumn){return{tokenize:tokenBase,
scopes:[{offset:basecolumn||0,type:"py",align:null}],indent:basecolumn||0,lastToken:null,lambda:!1,dedent:0}},token:function(stream,state){var addErr=state.errorToken;addErr&&(state.errorToken=!1);a:{stream.sol()&&(state.beginningOfLine=!0,state.dedent=!1);var style=state.tokenize(stream,state);var current=stream.current();if(state.beginningOfLine&&"@"==current)style=stream.match(identifiers,!1)?"meta":py3?"operator":"error";else{/\S/.test(current)&&(state.beginningOfLine=!1);"variable"!=style&&"builtin"!=
style||"meta"!=state.lastToken||(style="meta");if("pass"==current||"return"==current)state.dedent=!0;"lambda"==current&&(state.lambda=!0);":"==current&&!state.lambda&&"py"==top(state).type&&stream.match(/^\s*(?:#|$)/,!1)&&pushPyScope(state);if(1==current.length&&!/string|comment/.test(style)){var delimiter_index="[({".indexOf(current);if(-1!=delimiter_index){delimiter_index="])}".slice(delimiter_index,delimiter_index+1);var align=stream.match(/^[\s\[\{\(]*(?:#|$)/,!1)?null:stream.column()+1;state.scopes.push({offset:state.indent+
hangingIndent,type:delimiter_index,align})}delimiter_index="])}".indexOf(current);if(-1!=delimiter_index)if(top(state).type==current)state.indent=state.scopes.pop().offset-hangingIndent;else{style="error";break a}}state.dedent&&stream.eol()&&"py"==top(state).type&&1<state.scopes.length&&state.scopes.pop()}}style&&"comment"!=style&&(state.lastToken="keyword"==style||"punctuation"==style?stream.current():style);"punctuation"==style&&(style=null);stream.eol()&&state.lambda&&(state.lambda=!1);return addErr?
style+" error":style},indent:function(state,textAfter){if(state.tokenize!=tokenBase)return state.tokenize.isString?CodeMirror.Pass:0;var scope=top(state);state=scope.type==textAfter.charAt(0)||"py"==scope.type&&!state.dedent&&/^(else:|elif |except |finally:)/.test(textAfter);return null!=scope.align?scope.align-(state?1:0):scope.offset-(state?hangingIndent:0)},electricInput:/^\s*([\}\]\)]|else:|elif |except |finally:)$/,closeBrackets:{triples:"'\""},lineComment:"#",fold:"indent"}});CodeMirror.defineMIME("text/x-python",
"python");CodeMirror.defineMIME("text/x-cython",{name:"python",extra_keywords:"by cdef cimport cpdef ctypedef enum except extern gil include nogil property public readonly struct union DEF IF ELIF ELSE".split(" ")})})}
