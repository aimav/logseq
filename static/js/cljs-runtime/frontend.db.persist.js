...
frontend.db.persist.rename_graph_BANG_ = (function frontend$db$persist$rename_graph_BANG_(old_repo,new_repo){
var old_key = frontend.db.conn.datascript_db(old_repo);
var new_key = frontend.db.conn.datascript_db(new_repo);
if(cljs.core.truth_(frontend.util.electron_QMARK_())){
console.error("rename-graph! is not supported in electron");

return frontend.idb.rename_item_BANG_(old_key,new_key);
} else {
return frontend.idb.rename_item_BANG_(old_key,new_key);
}
});
