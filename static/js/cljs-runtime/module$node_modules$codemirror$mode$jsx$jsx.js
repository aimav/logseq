...
xmlMode)return xmlToken(stream,state,state.context);var cx=state.context;if("\x3c"==stream.peek()&&jsMode.expressionAllowed(stream,cx.state))state.context=new Context(CodeMirror.startState(xmlMode,jsMode.indent(cx.state,"","")),xmlMode,0,state.context),jsMode.skipExpression(cx.state),state=null;else{var style=jsMode.token(stream,cx.state);style||null==cx.depth||(stream=stream.current(),"{"==stream?cx.depth++:"}"==stream&&0==--cx.depth&&(state.context=state.context.prev));state=style}return state}
